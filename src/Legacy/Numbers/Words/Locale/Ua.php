<?php

namespace NumberToWords\Legacy\Numbers\Words\Locale;

class Ua extends Ru
{
    const LOCALE = 'ua';
    const LANGUAGE_NAME = 'Ukrainian';
    const LANGUAGE_NAME_NATIVE = 'Українська';
    const MINUS = 'мінус';

    protected $zero = 'нуль';

    protected static $ten = [
        ['', 'один', 'два', 'три', 'чотири', 'п\'ять', 'шість', 'сім', 'вісім', 'дев\'ять'],
        ['', 'одна', 'дві', 'три', 'чотири', 'п\'ять', 'шість', 'сім', 'вісім', 'дев\'ять'],
    ];

    protected static $teens = [
        'десять',
        'одинадцять',
        'дванадцять',
        'тринадцять',
        'чотирнадцять',
        'п\'ятнадцять',
        'шістнадцять',
        'сімнадцять',
        'вісімнадцять',
        'дев\'ятнадцять',
    ];

    protected static $tens = [
        2 => 'двадцять',
        'тридцять',
        'сорок',
        'п\'ятдесят',
        'шістдесят',
        'сімдесят',
        'вісімдесят',
        'дев\'яносто',
    ];

    protected static $hundred = [
        '',
        'сто',
        'двісті',
        'триста',
        'чотириста',
        'п\'ятсот',
        'шістсот',
        'сімсот',
        'вісімсот',
        'дев\'ятсот',
    ];

    protected static $mega = [
        [3 => self::FEMALE],
        [3 => self::MALE],
        ['тисяча', 'тисячі', 'тисяч', self::FEMALE],
        ['мільйон', 'мільйони', 'мільйонів', self::MALE],
        ['мільярд', 'мільярди', 'мільярдів', self::MALE],
        ['трильйон', 'трильйони', 'трильйонів', self::MALE],
        ['квадрильйон', 'квадрильйони', 'квадрильйонів', self::MALE],
        ['секстильйон', 'секстильйони', 'секстильйонів', self::MALE],
    ];

    protected static $currencyNames = [
        'AED' => [
            [0, 'дирхам ОАЕ', 'дирхама ОАЕ', 'дирхамів ОАЕ'],
            [1, 'філс', 'філса', 'філсів'],
        ],
        'ALL' => [
            [1, 'лік', 'ліки', 'ліків'],
            [2, 'кіндарка', 'кіндарки', 'кіндарок'],
        ],
        'AUD' => [
            [1, 'австралійський долар', 'австралійських долара', 'австралійських доларів'],
            [1, 'цент', 'цента', 'центів'],
        ],
        'BAM' => [
            [1, 'конвертована марка', 'конвертованих марки', 'конвертованих марок'],
            [1, 'фенінг', 'фенінги', 'фенінгів'],
        ],
        'BGN' => [
            [1, 'лев', 'ліва', 'лівий'],
            [2, 'стотинка', 'стотинки', 'стотинок'],
        ],
        'BRL' => [
            [1, 'бразильський реал', 'бразильських реалів', 'бразильських реалів'],
            [1, 'сентаво', 'сентаво', 'сентаво'],
        ],
        'BYR' => [
            [1, 'білоруський рубль', 'білоруських рубля', 'білоруських рублів'],
            [2, 'копійка', 'копійки', 'копійок'],
        ],
        'CAD' => [
            [1, 'канадський долар', 'канадських долара', 'канадських доларів'],
            [1, 'цент', 'цента', 'центів'],
        ],
        'CHF' => [
            [1, 'швейцарський франк', 'швейцарських франків', 'швейцарських франків'],
            [1, 'сантим', 'сантима', 'сантимів'],
        ],
        'CNY' => [
            [0, 'китайський юань', 'китайських юаня', 'китайських юанів'],
            [0, 'цзяо', 'цзяо', 'цзяо'],
        ],
        'CYP' => [
            [1, 'кіпрський фунт', 'кіпрських фунтів', 'кіпрських фунтів'],
            [1, 'цент', 'цента', 'центів'],
        ],
        'CZK' => [
            [2, 'чеська крона', 'чеських крони', 'чеських крон'],
            [1, 'Галірж', 'Галіржа', 'Галіржа'],
        ],
        'DKK' => [
            [2, 'датська крона', 'датських крони', 'датських крон'],
            [1, 'ері', 'ері', 'ері'],
        ],
        'DZD' => [
            [0, 'алжирський динар', 'алжирських динара', 'алжирських динарів'],
            [1, 'сантим', 'сантима', 'сантимів'],
        ],
        'EEK' => [
            [2, 'естонська крона', 'естонські крони', 'естонські крони'],
            [1, 'сенти', 'сенти', 'сенти'],
        ],
        'EGP' => [
            [0, 'єгипетський фунт', 'єгипетських фунта', 'єгипетських фунтів'],
            [1, 'піастр', 'піастра', 'піастрів'],
        ],
        'EUR' => [
            [1, 'євро', 'євро', 'євро'],
            [1, 'євроцент', 'євроцента', 'євроцентів'],
        ],
        'GBP' => [
            [1, 'фунт стерлінгів', 'фунт стерлінгів', 'фунт стерлінгів'],
            [1, 'пенс', 'пенса', 'пенсів'],
        ],
        'HKD' => [
            [1, 'гонконзький долар', 'гонконзький долар', 'гонконзький долар'],
            [1, 'цент', 'цента', 'центів'],
        ],
        'HRK' => [
            [2, 'хорватська куна', 'хорватські куни', 'хорватських кун'],
            [2, 'липа', 'липи', 'лип'],
        ],
        'HUF' => [
            [1, 'угорський форинт', 'угорських форинтів', 'угорських форинтів'],
            [1, 'філер', 'філера', 'філерів'],
        ],
        'IDR' => [
            [1, 'індонезійська рупія', 'індонезійських рупії', 'індонезійських рупій'],
            [1, 'сен', 'сена', 'сенів'],
        ],
        'ILS' => [
            [0, 'ізраїльський шекель', 'ізраїльських шекелі', 'ізраїльських шекелів'],
            [1, 'агора', 'агори', 'агор'],
        ],
        'ISK' => [
            [2, 'ісландська крона', 'ісландські крони', 'ісландські крони'],
            [1, 'ері', 'ері', 'ері'],
        ],
        'JPY' => [
            [2, 'єна', 'єни', 'єн'],
            [2, 'сіна', 'сіни', 'сін'],
        ],
        'LTL' => [
            [1, 'літ', 'літа', 'літів'],
            [1, 'цент', 'цента', 'центів'],
        ],
        'LVL' => [
            [1, 'лат', 'лата', 'латів'],
            [1, 'сентим', 'сентима', 'сентимів'],
        ],
        'LYD' => [
            [0, 'лівійський динар', 'лівійських динара', 'лівійських динарів'],
            [1, 'дирхам', 'дирхама', 'дирхамів'],
        ],
        'MAD' => [
            [0, 'марокканський дирхам', 'марокканських дирхама', 'марокканських дирхамів'],
            [1, 'сантим', 'сантима', 'сантимів'],
        ],
        'MKD' => [
            [1, 'македонський динар', 'македонських динарів', 'македонських динарів'],
            [1, 'дені', 'дені', 'дені'],
        ],
        'MXN' => [
            [1, 'мексиканський песо', 'мексиканських песо', 'мексиканських песо'],
            [1, 'сентаво', 'сентаво', 'сентаво'],
        ],
        'MRO' => [
            [1, 'мавританська угія', 'мавританських угії', 'мавританських угій'],
            [0, 'хум', 'хума', 'хумів'],
        ],
        'MTL' => [
            [2, 'мальтійська ліра', 'мальтійські ліри', 'мальтійські ліри'],
            [1, 'сентим', 'сентима', 'сентимів'],
        ],
        'MYR' => [
            [0, 'малайзійський рингіт', 'малайзійських рингіта', 'малайзійських рингітів'],
            [1, 'сен', 'сена', 'сенів'],
        ],
        'NGN' => [
            [1, 'нігерійська найра', 'нігерійських найри', 'нігерійських найр'],
            [0, 'кобо', 'кобо', 'кобо'],
        ],
        'NOK' => [
            [2, 'норвезька крона', 'норвезькі крони', 'норвезьких крон'],
            [0, 'ері', 'ері', 'ери'],
        ],
        'PHP' => [
            [2, 'філіппінське песо', 'філіппінських песо', 'філіппінських песо'],
            [1, 'сентаво', 'сентаво', 'сентаво'],
        ],
        'PLN' => [
            [1, 'злотий', 'злотих', 'злотих'],
            [1, 'гроші', 'гроші', 'грошей'],
        ],
        'RON' => [
            [1, 'румунський лей', 'румунських лей', 'румунських лей'],
            [1, 'бані', 'бані', 'бані'],
        ],
        // both RUR і RUR є використані, деякі users use RUB for shorter form
        'RUB' => [
            [1, 'рубль', 'рубля', 'рублів'],
            [2, 'копійка', 'копійки', 'копійок'],
        ],
        'RUR' => [
            [1, 'російський рубль', 'російських рубля', 'російських рублів'],
            [2, 'копійка', 'копійки', 'копійок'],
        ],
        'SAR' => [
            [0, 'саудівський ріял', 'саудівських ріяла', 'саудівських ріялів'],
            [1, 'філс', 'філса', 'філсів'],
        ],
        'SEK' => [
            [2, 'шведська крона', 'шведські крони', 'шведські крони'],
            [1, 'ері', 'ері', 'ері'],
        ],
        'SIT' => [
            [1, 'словенський толар', 'словенських толарів', 'словенських толарів'],
            [2, 'стотин', 'стотин', 'стотин'],
        ],
        'SKK' => [
            [2, 'словацька крона', 'словацьких крони', 'словацьких крон'],
            [0, '', '', ''],
        ],
        'TMT' => [
            [0, 'туркменський манат', 'туркменських маната', 'туркменських манатів'],
            [1, 'тенге', 'тенге', 'тенге'],
        ],
        'TND' => [
            [0, 'туніський динар', 'туніських динара', 'туніських динарів'],
            [1, 'міліс', 'мілісів', 'мілісів'],
        ],
        'TRL' => [
            [2, 'турецька ліра', 'турецькі ліри', 'турецькі ліри'],
            [1, 'піастр', 'піастра', 'піастрів'],
        ],
        'TRY' => [
            [2, 'турецька ліра', 'турецькі ліри', 'турецькі ліри'],
            [1, 'піастр', 'піастра', 'піастрів'],
        ],
        'UAH' => [
            [2, 'гривня', 'гривні', 'гривень'],
            [2, 'копійка', 'копійки', 'копійок'],
        ],
        'USD' => [
            [1, 'долар США', 'долара США', 'доларів США'],
            [1, 'цент', 'цента', 'центів'],
        ],
        'UZS' => [
            [0, 'узбецький сум', 'узбецьких сума', 'узбецьких сумів'],
            [1, 'тийїн', 'тийїна', 'тийїнів'],
        ],
        'XAF' => [
            [0, 'франк КФА', 'франки КФА', 'франків КФА'],
            [1, 'сантим', 'сантима', 'сантимів'],
        ],
        'XOF' => [
            [0, 'франк КФА', 'франки КФА', 'франків КФА'],
            [1, 'сантим', 'сантима', 'сантимів'],
        ],
        'XPF' => [
            [0, 'франк КФП', 'франки КФП', 'франків КФП'],
            [1, 'сантим', 'сантима', 'сантимів'],
        ],
        'YUM' => [
            [1, 'югославський динар', 'югославських динарів', 'югославських динарів'],
            [1, 'пара', 'пара', 'пара'],
        ],
        'ZAR' => [
            [1, 'ранд', 'ранда', 'рандів'],
            [1, 'цент', 'цента', 'центів'],
        ]
    ];
}
