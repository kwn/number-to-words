...
class Yo extends Words
{
	...
	protected function toWords($num)
	{
		...
		foreach ($split as $key => $number) {
		    if(!$number) continue;
			$suffix = $this->suffix($number); //2
			$prefix = $this->prefix(count($split) - 1); //million
            $combined = trim($prefix.' '.$suffix);

			$word .= (count($split) == 1 && !empty($word) ? " " . self::$conjunction . " " : !empty($word) ? ", " : "") . $combined;
			unset($split[$key]);
		}
	...
	private function suffix($num)
	{
		...
		if (strlen($num) == 2 && $num > 19) {
			$ten = $num - ($num % 10);
			$word .= ($word && !empty(self::$tens[$ten]) ? " " . self::$conjunction . " " : '') . self::$tens[$ten];

			$num = (int) substr($num, 1);
		}

		if (strlen($num) == 2 && $num <= 19) {
			$word .= ($word && !empty(self::$teens[$num]) ? " " . self::$conjunction . " " : '') . self::$teens[$num];

			$num = str_replace($num, "", $num);
		}

		if (strlen($num) == 1) {
			$word .= ($word && !empty(self::$unit[$num]) ? " " . self::$conjunction . " " : '') . self::$unit[$num];
		}
	...
	public function toCurrencyWords($currency, $decimal, $fraction = null)
	{
		...
		if ($fraction) {
			...
			$word .= " ".self::$separator . " " . self::$currencyNames[$currency][1][0]. " ".$decimal ;
		}
